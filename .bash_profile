...
psql-replica () {
	...
	__run-psql "$_RO_USER" "$_RO_PASSWORD" "$_REPLICA_HOST" "$_RO_DB" "$3"
}
...
psql-dump-replica () {
	...
	__run-psql-dump "$_RO_USER" "$_RO_PASSWORD" "$_REPLICA_HOST" "$_RO_DB" "\
		-n parcels \
		-n payments \
		-n constraints \
		-n contacts \
...
psql-dump-for () {
	...
	__run-psql-dump "$_USER" "$_PASSWORD" "$_HOST" "$_DB" "-n $_SCHEMA"
}
